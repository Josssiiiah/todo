...
export async function doTheAuthThing({
  context,
  request,
}: Pick<LoaderFunctionArgs, "request" | "context">) {
  // check out my typescript skill 8=====D
  const db = drizzle(context.cloudflare.env.Dormpages_DB);
  ...
  const lucia = initializeLucia(context.cloudflare.env.Dormpages_DB);
  ...
  if (result.session && result.session.fresh) {
    // this means the user was logged in with a fresh session already
    let sessionCookie = lucia.createSessionCookie(result.session.id);
  }

  if (!result.session) {
    // this means the user's session wasn't fresh
